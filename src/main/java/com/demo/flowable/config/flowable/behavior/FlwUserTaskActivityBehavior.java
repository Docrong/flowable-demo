...
import com.demo.flowable.service.impl.FlwTaskServiceImpl;
 ...
 */
@Slf4j
public class FlwUserTaskActivityBehavior extends UserTaskActivityBehavior {


    public FlwUserTaskActivityBehavior(UserTask userTask) {
        super(userTask);
    }
    ...
    protected void handleAssignments(TaskService taskService, String assignee, String owner,
        ...
        if (execution.getCurrentActivityId().startsWith("UserTaskAdd")) {
    ...
    private String calculateTaskCandidateUsers(DelegateExecution execution) {
        ...
        if (super.multiInstanceActivityBehavior != null) {
            return execution.getVariable(super.multiInstanceActivityBehavior.getCollectionElementVariable()).toString();
        }

        // 情况二，如果非多实例的任务，则计算任务处理人
        // 第一步，先计算可处理该任务的处理人们
        Set<String> candidateUserIds = CollUtil.newHashSet(StringUtil.nullObject2Str(execution.getVariable(FlwTaskServiceImpl.assigneeId)));
        // 第二步，后随机选择一个任务的处理人
        // 疑问：为什么一定要选择一个任务处理人？
